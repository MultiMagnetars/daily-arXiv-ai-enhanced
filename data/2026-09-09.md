...
### [1] [A Missing Tool for Calculating Auto/Cross-correlation Function under Nonuniform Sampling Observations](https://arxiv.org/abs/2609.08604)
*Hu, Chen-Ran,Huang, Yong-Feng,Geng, Jin-Jun,Amat, Orkash,Zou, Ze-Cheng,Deng, Chen,Xu, Fan,Dong, Xiao-Fei,Du, Chen,Nurmamat, Nurimangul,Wang, Pei,Cui, Lang,Li, Cheng-Ming*

Main category: scix

TL;DR: 研究者提出一种计算非均匀自相关函数（NUACF）与非均匀互相关函数（NUCCF）的新方法，通过时间间隔权重和错位惩罚而非插值来评估相关函数。模拟和真实数据集（恒星光变、AGN Fairall 9 盘反响、引力透镜类星体 HE 0435-1223）表明其优于传统方法，且框架可直接适用于包括快速射电暴（FRB）在内的时域现象。


<details>
  <summary>Details</summary>
Motivation: 非均匀采样使标准自相关与互相关函数失效，而现有的插值或分箱等临时方法会引入未量化的偏差并缺乏严格的误差估计。本文旨在提供一种严谨、通用且在误差评估上更完善的方法来计算非均匀采样时间序列的相关函数。

Method: 提出一种新方法，不依赖插值，而是通过纳入时间间隔权重与错位惩罚来评估非均匀自相关函数（NUACF）与非均匀互相关函数（NUCCF）。采用蒙特卡罗模拟提供显著性评估的置信带与时间延迟的误差预算（同时考虑流量不确定性和采样不规则性），并通过大规模模拟及多种真实天体物理数据集（恒星光变曲线、AGN Fairall 9、引力透镜类星体 HE 0435-1223）验证。摘要未说明具体观测频率与时间跨度。

Result: 通过大量模拟表明，该方法在从严格周期性到复杂重复性（如间歇但非周期性）等各种条件下均优于传统方法。真实数据集验证显示其能揭示恒星光变曲线中的重复性变化、测量 AGN Fairall 9 多波段盘反响的时间延迟，并提供 HE 0435-1223 引力透镜时间延迟的模型无关验证。该方法提供了一种针对非均匀采样的严谨而通用的解法。

Conclusion: 作者将所提方法定位为非均匀采样相关计算问题的严谨通用解决方案，可作为大规模时域巡天数据分析的有用工具。摘要还指出该框架可直接适用于 FRB 等新兴时域现象，例如研究持久射电源光度与重复性 FRB 活动间的相关性。注意：这是一种方法论声明，摘要未提供针对脉冲星或磁星的具体实测结论。
...
### [2] [Axion Hair and Pulsar Electrodynamics: modelling, discharge dynamics, and particle-in-cell simulations](https://arxiv.org/abs/2609.08840)
*Witte, Samuel J.,Caputo, Andrea,Stelzl, Stefan,Chernoglazov, Alexander,Philippov, Alexander A.,Rajendran, Surjeet*

Main category: scix

TL;DR: 本文扩展了轴子毛发对年老自转供能脉冲星极隙区电动力学影响的理论与数值框架，表明轴子毛发要么增强粒子加速、要么增强屏蔽，且该效应在星的南北半球及支撑外流/回流电流的场线之间存在差异。


<details>
  <summary>Details</summary>
Motivation: 在配套论文中已证明轴子毛发可主导年老旋转脉冲星的近场电动力学并产生新约束，本文提供支撑该结果的理论与数值框架。研究间隙在于理解轴子场梯度如何具体修改极隙区的放电与电动力学，为后续约束轻QCD轴子及破坏CP的轴子-核子相互作用奠定基础。

Method: 采用解析与数值相结合的方法：先对稠密核物质产生的轴子毛发在不同相互作用参数空间计算自洽场轮廓；再在解析层面考察轴子场梯度对有效放电参数的修正，此参数在分裂单极磁场的无力极限下计算，并考察偶极场构型相对无力极限的首阶偏离；同时在数值层面发展专用的1D粒子网格（particle-in-cell）模拟以捕捉星体附近的首阶动力学行为。摘要未说明具体观测数据或仪器。

Result: 结果表明，轴子毛发的作用要么增强粒子加速、要么增强屏蔽；相关效应在星体北半球与南半球之间、以及在支撑外流电流与回流电流的场线之间会有所不同。本文是对配套论文所报道轻QCD轴子及破坏CP轴子-核子相互作用约束之理论数值框架的扩展描述，摘要未给出具体数值或统计显著性。

Conclusion: 作者明确支持的结论是：轴子毛发可增强粒子加速或增强屏蔽，且该效应呈现半球与电流方向相关的空间差异，从而支撑年老自转供能脉冲星近场电动力学可对轻QCD轴子及破坏CP的轴子-核子相互作用形成新的约束。由于摘要未论述与磁星或FRB的直接联系，对此类对象及计时、偏振等主题的转用根据摘要无法判断。
...
### [3] [The youngest white dwarf companion to a millisecond pulsar: Insights from NGC 362D](https://arxiv.org/abs/2609.05608)
*Ettorre, Greta,Dalessandro, Emanuele,Cadelano, Mario,Ridolfi, Alessandro,Pallanca, Cristina,Freire, Paulo C. C.,Venkatraman Krishnan, Vivek,Salaris, Maurizio,D'Antona, Franca,Nag, Rouhin*

Main category: scix

TL;DR: 作者在球状星团NGC362中识别出毫秒脉冲星NGC362D的光学对应体，认定其为一颗仍在预冷却阶段的$\sim0.18 M_{\odot}$氦白矮星，并推断它约0.6 Gyr前才完成质量转移，是迄今已知最年轻的MSP白矮星伴星，其光度首次表现出与残余星周物质一致的波长相关变化。


<details>
  <summary>Details</summary>
Motivation: 本文旨在识别新近发现的毫秒脉冲星NGC362D在球状星团NGC362中的光学对应体，并约束MSP再循环过程及原白矮星伴星的最早期演化阶段，填补年轻MSP伴星及其射电与光学特性认识的空白。

Method: 依据摘要，作者基于哈勃空间望远镜（Hubble Space Telescope）的深场、多波段、多历元观测进行光学对应体识别，并结合光度分析确定天体性质，再通过更新的双星演化模型与观测做详细比较，约束质量转移完成时间。

Result: 该对象被稳健识别为极低质量（$\sim0.18 M_{\odot}$）氦白矮星，仍处于预冷却阶段；详细比较表明其在约$\sim0.6$ Gyr前刚完成质量转移，强有力地显示COM-NGC362D是迄今最年轻的MSP白矮星伴星；其光度首次表现出显著的波长相关变化，与残余星周物质的存在一致。

Conclusion: 作者认为该系统为直接探测MSP再循环过程的直接后果、约束原白矮星伴星的早期演化及射电与光学特性提供了有价值检验案例；残余物质可能产生与非简并伴星系统相似的射电与光学特征，导致年轻MSP被误分类。根据摘要无法判断其与磁星、FRB或脉冲计时等方向的直接关联。
...
### [4] [Search for Long-Transient Gravitational Waves from Supernova SN2023ixf using GFH-v2 Pipeline](https://arxiv.org/abs/2609.07774)
*Sajith Menon, Sandhya,Pierini, Lorenzo,Astone, Pia,Palomba, Cristiano,Safai Tehrani, Francesco,Silvestri, Lorenzo,Juliana Piccinni, Ornella,Dall'Osso, Simone,Dal Pra, Stefano,D'Antonio, Sabrina,Frasca, Sergio,Guetta, Dafne,Leaci, Paola,Bejger, Michal,Calafat, Alicia,Goetz, Evan,Jaume Amengual, Rafel,Jones, Ian,Keitel, David,Krolak, Andrzej,La Rosa, Iuri,Melatos, Andrew,Merou, Joan-Rene,Mirasola, Lorenzo,Poggiani, Rosa,Salvadore, Claudio,Sintes, Alicia M.,Wette, Karl,Zanolin, Michele*

Main category: scix

TL;DR: 针对超新星SN2023ixf可能形成的年轻磁星遗留体开展的长时标瞬态引力波定向搜索未发现候选体；最高椭率区间下90%上限可达约1–2.5 Mpc，低于M101的距离，同时完成GFH-v2对近距核塌缩超新星的首次应用。


<details>
  <summary>Details</summary>
Motivation: 作者聚焦于近距II型核塌缩超新星SN2023ixf（位于M101星系）可能遗留的年轻磁星；此类快速旋转非轴对称中子星的自转减慢若由引力波辐射主导，会产生可探测的长时标瞬态引力波信号，本文旨在以其为目标的定向搜索中验证信号的探测能力。

Method: 摘要说明该分析使用LIGO Hanford和Livingston在Engineering Run 15期间的数据，采用位于超新星on-source窗口内的符合数据；搜索针对频率呈幂律下降、应变幅值相应下降的快速旋转非轴对称中子星信号，使用基于广义频率Hough变换的GFH-v2流水线执行，并配合符合和后续分析。

Result: 搜索中无候选体通过符合和后续分析；进而给出了作为初始频率和椭率函数的最大可探测距离上限。对于最高椭率区间，90%上限在分析的大部分频带内达到约1–2.5~Mpc。这些上限低于M101的距离，且该搜索为GFH-v2首次应用于近距核塌缩超新星，并刻画了其在真实探测器数据上的性能。

Conclusion: 在SN2023ixf可能的年轻磁星遗留体的长时标引力波定向搜索中未发现稳健候选体，所得距离上限低于M101距离；作者将其定位为GFH-v2流水线对近距核塌缩超新星的首次应用及其在真实探测器数据上性能的刻画。根据摘要无法判断该结果与研究脉冲星、磁星及FRB的其他观测之间的物理关联或可迁移性。
...
### [5] [A Unified Magnetohydrodynamic Scaling Relation for the Multiphase Interstellar Medium](https://arxiv.org/abs/2609.06900)
*Zhao, Mengke,Li, Guang-Xing,Qiu, Keping*

Main category: scix

TL;DR: 本文通过合并脉冲星与塞曼观测数据，构建了跨越十个量级密度的统一数据集，表明星际介质的磁场—密度关系可用连续磁流体演化描述，并以$\mathcal{M}_{\rm A}=1$定界的经验转折密度作为分段幂律的近似。


<details>
  <summary>Details</summary>
Motivation: 磁场、湍流与引力共同支配星际介质结构演化与恒星形成初始条件，但观测空白长期迫使磁场—密度关系采用分段幂律描述。本文试图以统一数据检验是否存在由阿尔芬马赫数组织的连续磁场演化。

Method: 汇编统一数据集，结合脉冲星与塞曼（Zeeman）观测，覆盖密度范围$10^{-26}$–$10^{-16}\,\mathrm{g\,cm^{-3}}$（跨越十个量级）。将数据与连续磁流体模型比较，检验渐变转变模型的三项拟合与外推预测，并采用尺度映射考察磁能谱标度。

Result: 统一数据与由$\mathcal{M}_{\rm A}=\sqrt{E_K/E_B}$组织的连续演化一致：低密度气体磁场主导（$\mathcal{M}_{\rm A}<1$），高密度气体动能主导（$\mathcal{M}_{\rm A}>1$），经验转折密度对应$\mathcal{M}_{\rm A}=1$。拟合指数$β\approx0.15$–$0.21$（预测$0.147$），背景场$B_c\approx2.0\,μ$G；致密气体拟合跨越四个量级外推穿过弥散脉冲星数据；磁能谱大尺度趋近$k^{-5/3}$类标度，小尺度偏离外推。

Conclusion: 作者认为分段幂律可视为连续磁状态方程的分段近似，拟合转变密度可能潜在地与引力驱动运动的开启存在物理联系。关于该关系对脉冲星、磁星或FRB计时等观测效应的直接适用性，根据摘要无法判断。
...
### [6] [Approximating the statistics of a gravitational wave background](https://arxiv.org/abs/2609.07686)
*Falxa, Mikel*

Main category: scix

TL;DR: 该文提出一种基于鞍点近似方法的工具，用以估计任何给定群体模型下引力波背景（GWB）特征应变的分布。利用理想模拟表明，正确建模高阶矩对于从GWB统计中推断SMBHB天体物理模型参数至关重要。


<details>
  <summary>Details</summary>
Motivation: 脉冲星计时阵列（PTA）报告GWB的天体物理起源尚未确认。由SMBHB群体单个引力波之和组成的GWB信号会携带离散泊松统计印记，从而偏离高斯性，因此需要对GWB统计进行建模以用于推断。

Method: 作者提出基于鞍点近似方法（saddlepoint approximation method）的工具估计给定群体模型下特征应变的分布；引入刻画重尾行为的方差混合高斯分布一般族；证明为高斯自由谱PTA似然设置正确分层先验等效于非高斯似然；并利用理想模拟比较鞍点近似与log-Normal分布的性能。摘要未说明频率、时间跨度和样本细节。

Result: 作者提出鞍点近似工具可用于贝叶斯推断或快速可视化来自半解析模型的GWB统计；引入方差混合高斯分布以刻画非高斯GWB信号的重尾行为；证明对高斯自由谱PTA似然设置正确分层先验等效于非高斯似然；理想模拟显示正确建模高阶矩至关重要。

Conclusion: 作者认为正确建模高阶矩对于从GWB统计中推断天体物理模型参数是必要的，并建议未来PTA分析应在其流程中纳入GWB的统计性质。
...
### [7] [Distinguishing lensing and precessional modulation in binary black-hole inspiral waveforms](https://arxiv.org/abs/2609.06347)
*Nguyen-Vo, Tien N.,Singh, Tamanjyot,McKallip, Benjamin,Kesden, Michael,King, Lindsay*

Main category: scix

TL;DR: 作者研究了单个L形引力波探测器区分双黑洞引力波中进动调制与透镜调制的可行性，发现当透镜图像间时间延迟足够短、干涉条纹少于三个时，进动调制可模拟透镜调制，因此排除进动调制对识别真正透镜系统至关重要。


<details>
  <summary>Details</summary>
Motivation: 区分引力波信号中的透镜调制与进动调制对于识别真正被强透镜化的双黑洞系统至关重要，因为强透镜在中等红移处罕见，而错向自旋导致的进动在特定形成通道产生的双黑洞中很常见。

Method: 摘要未说明具体观测数据或仪器频率。摘要指出该研究针对单个L形引力波探测器，考察其区分两种调制来源（引力透镜多像干涉调制与错向自旋引起的轨道角动量绕总角动量进动调制）的能力，并检验干涉条纹数目与时间延迟等条件。

Result: 研究发现，当透镜两像之间时间延迟足够短，使引力波信号停留在探测器灵敏度带宽期间发生少于三个干涉条纹时，进动调制可以模拟具有可比放大率的两个像之间的透镜调制。该结果揭示了这两种调制在特定条件下难以区分。

Conclusion: 作者认为，由于强透镜对于中等红移处的引力波源较为罕见，而错向自旋在特定形成通道产生的双黑洞中很常见，因此排除进动调制是识别真正透镜系统的关键所在。根据摘要无法判断。
...
### [8] [Alternative neural-network follow-up for all-sky FrequencyHough in continuous gravitational-wave searches](https://arxiv.org/abs/2609.06211)
*Di Cesare, Martina,Astone, Pia,De Rosa, Rosario,Keitel, David,Palomba, Cristiano,Serra, Marco*

Main category: scix

TL;DR: 本文提出一种将神经网络分类器纳入孤立中子星全天连续引力波搜索的新方法。该方法在LIGO和Virgo第三次观测运行（O3）数据上无需增加计算成本即可提高灵敏度，即使低应变下也能稳健区分信号与噪声，并成功识别训练自转减慢范围之外的硬件注入。


<details>
  <summary>Details</summary>
Motivation: 银河系预计拥有$10^{8}-10^{9}$颗中子星，但目前只有约$10^{3}$颗通过电磁观测被发现，这一巨大差距表明存在大量电磁学沉默或未被发现的中子星。基于连续引力波的全天搜索可在无先验信息情况下探测这一隐藏群体，本文旨在改进其灵敏度。

Method: 提出将神经网络（NN）分类器纳入基于FrequencyHough流水线的全天搜索策略。分析LIGO和Virgo等地面探测器O3数据（2019年4月1日至2020年3月27日），频率范围[129, 229] Hz，自转减慢区间[$-2.5\cdot10^{-9}, 1.0\cdot10^{-9}$] Hz/s。模型在与真实O3数据统计一致的噪声上训练，在真实O3数据上测试；方法可跨多探测器并行化。

Result: 结果表明该方法在不增加计算成本的情况下提高了灵敏度。模型在区分信号与噪声方面表现出稳健性能，即使在低应变下也能有效工作，并成功识别了训练自转减慢范围之外的硬件注入。该算法可跨多个探测器并行化，以增强探测概率并减少误报。

Conclusion: 作者认为该神经网络方法能有效提高全天连续引力波搜索的灵敏度，可探测电磁学沉默或未被发现的中子星，从而探测银河系隐藏的中子星群体。摘要未涉及脉冲星、磁星或FRB的具体连接，作为一种备用信使搜索方法，其与脉冲星群体的关系根据摘要无法判断。
...
### [9] [LOFAR follow-up of sources in the First LHAASO Catalogue](https://arxiv.org/abs/2609.07119)
*Maria Arias,Timothy Shimwell,Martin Hardcastle,Roland Timmerman*

Main category: astro-ph.HE

TL;DR: 利用LOFAR LoTSS-DR3的144 MHz连续谱图像，作者搜寻55个1LHAASO银河系源的射电对映体，报告24个新的SNR候选体，并估计至少四分之一至一半的银河系平面1LHAASO源真正位于SNR环境中，支持以脉冲星风云、SNR-PWN复合体等为主导的族群。


<details>
  <summary>Details</summary>
Motivation: 首个LHAASO目录列出90个甚高能伽马射线源，其中许多在射电等其他频率上没有已确立的对映体。这重新提出了哪些银河系加速器产生最高能宇宙射线的问题，故需要确定这些伽马射线源的射电对应结构及其所处的SNR环境。

Method: 针对落入LOFAR LoTSS-DR3银河系天区的55个1LHAASO源，使用144~MHz连续谱图像（分辨率$6''$和$20''$）进行搜寻，并在可用时辅以其他射电巡天的档案数据。采用自举（bootstrap）分析评估与已编目SNR的随机重合概率，并对射电形态与红外、伽马射线发射进行对照识别。

Result: 报告24个新的SNR候选体、9个具有微弱或无红外对映体的平谱射电壳层，以及6个无明显银河系射电发射的源。对$|b| \leq 5$的50个银河系平面源，23个（46\%）与已编目SNR重叠，而随机放置即使假设SNR族群扩大十倍也仅得$10.0\pm2.8$个匹配；外银河系12个源中有4个重合SNR，空期望为$\lesssim2$。估计至少四分之一至一半的1LHAASO银河系平面源位于SNR环境中。

Conclusion: 作者推断这些甚高能伽马射线源中相当比例（至少四分之一、至多一半）真正位于SNR环境中，与由脉冲星风云、SNR--PWN复合体及宇宙射线照射云团主导的族群一致。并给出个别候选体，包括HESS~J1912+101内的新SNR候选等。根据摘要无法判断对磁星或FRB的直接关联结论。
...
### [10] [Timing Gravity with Pulsars in the Strong Field](https://arxiv.org/abs/2609.07412)
*Riccardo Della Monica,Ivan de Martino*

Main category: astro-ph.HE

TL;DR: 作者提出一种将光子传播时间完全相对论计算纳入计时模型的脉冲星计时新方法，并以银河系中心Sgr A*周围的假定脉冲星为对象开展概念验证灵敏度分析，指出这类观测可将黑洞质量约束改善至少三个数量级。


<details>
  <summary>Details</summary>
Motivation: 超强引力场区域（如银河系中心超大质量黑洞Sgr A*附近）目前缺乏高精度测量手段。本文旨在通过围绕该黑洞运行的脉冲星计时，实现对Sgr A*物理性质（尤其质量）的更精确表征，以超越S星与视界尺度观测的现有约束。

Method: 摘要说明提出一种将完全相对论光子传播时间计算纳入稳健计时模型的脉冲星计时方法；生成针对多颗假定绕Sgr A*紧致轨道运行的脉冲星的模拟到达时间（times-of-arrival）星表，并开展概念验证式灵敏度分析，用以预测平方公里阵列（Squared Kilometer Array）等未来设施的表征精度。具体观测设施、频率等细节摘要未说明。

Result: 分析表明，对银河系中心Sgr A*附近的脉冲星进行观测，可为表征Sgr A*的物理性质开辟极具前景的途径，其可将目前通过S星和视界尺度观测所获得的黑洞质量约束改善至少三个数量级。摘要未给出具体数值精度或显著性统计。

Conclusion: 作者推断，银河系中心脉冲星的计时观测将能以前所未有的精度表征Sgr A*的物理性质，较现有基于S星与视界尺度观测的质量约束提升至少三个数量级。根据摘要无法判断该方法是否可直接推广用于磁星或快速射电暴（FRB）等研究。
...
### [11] [Mining the time axis with TRON - III. Discovery of a nulling pulsar towards the SMC with MeerKAT](https://arxiv.org/abs/2609.07528)
...
TL;DR: 利用TRON图像平面瞬变检测流水线在小麦哲伦云（SMC）方向的MeerKAT观测中发现了一颗消零脉冲星PSR J0052$-$7551，其自转周期为0.55 s，消零比例约32%。这是TRON首次由消零行为触发的脉冲星探测。


<details>
  <summary>Details</summary>
Motivation: 论文旨在利用TRON图像平面瞬变检测流水线从存档数据中发现新的非闪烁、非掩食类型的中等时间尺度变源。TRON此前依赖闪烁或掩食行为探测脉冲星，本文探索其能否通过消零行为发现源，以证实该方法对广泛瞬变现象（包括消零脉冲星）的敏感性。

Method: 使用TRON图像平面瞬变检测流水线对SMC方向的MeerKAT L波段存档观测进行分析；TRON为每个时间样本生成残差图像，并采用启发式剪枝策略在时间图像立方体中识别候选体。对触发的候选体进行MeerKAT UHF波段波束成形后续观测。采用高斯混合模型拟合消零行为，并对UHF可见度数据进行功率谱分析。

Result: 发现消零脉冲星PSR J0052$-$7551，自转周期0.55 s，周期导数6.1(6)$\times$10$^{-17}$s$\cdot$s$^{-1}$，位于SMC外围投影方向，色散量值29.24(2) pc$\cdot$cm$^{-3}$（与银河系贡献同量级）。高斯混合模型拟合得到消零比例约32%，消零长度符合指数分布。UHF可见度数据功率谱显示一个无法归因于源、RFI或仪器效应的未知准周期特征（约5$-$7 s）。

Conclusion: 本文作者得出结论：PSR J0052$-$7551的消零触发探测是TRON方法首次由消零行为实现的脉冲星发现，继此前闪烁和掩食探测之后，证实TRON对中等时间尺度持续时间的多种天体物理现象具有敏感性。功率谱中约5$-$7 s的准周期特征无法解释，其物理性质根据摘要无法判断。
...
### [12] [Interior Magnetic Fields in Magnetars and Radio Pulsars](https://arxiv.org/abs/2609.07647)
*Raj Kishor Joshi,Brynmor Haskell,William Cook,Sebastiano Bernuzzi*

Main category: astro-ph.HE

TL;DR: 利用三维广义相对论磁流体动力学模拟，论文表明中子星的内部磁场结构敏感地依赖于其旋转状态：旋转主导的恒星（如射电脉冲星）形成强的环向磁场，而磁星则以极向为主的构型为特征，其旋转历史由此塑造内部磁场结构。


<details>
  <summary>Details</summary>
Motivation: 磁场在驱动磁星极端现象中发挥核心作用，但其内部磁场结构与稳定性在很大程度上仍不受约束，且无法通过电磁观测直接探测。论文旨在用三维GR-MHD模拟研究内部磁场随旋转速率与场强度演化的机制。

Method: 摘要未说明望远镜或观测方法。论文采用三维广义相对论磁流体动力学（GR-MHD）模拟，跨越一系列旋转速率和磁场强度，分析恒星内部磁场从初始状态向动力学稳定构型的演化过程及其特征时标。

Result: 识别出两个演化机制：在快速旋转恒星中，Alfvén穿越时标超过旋转周期，差速缠绕在失稳前放大强环向磁场，形成长寿命稳定构型；在磁主导恒星中，极向场不稳定性驱动快速场衰减，仅留下较弱的环向分量。因此旋转主导的射电脉冲星具有强环向场，磁星以极向为主的构型为特征。

Conclusion: 作者认为中子星的旋转历史塑造其内部磁场结构：旋转主导恒星（射电脉冲星）形成强环向场，磁星为极向为主构型，从而提供一个将中子星类多样性与隐藏场构型相联系的统一物理图景。关于按摘要无法判断对单脉冲、计时等具体观测的应用。
...
### [13] [Bridging the Population Synthesis of Supermassive Binary Black Holes and the Gravitational Wave Background](https://arxiv.org/abs/2609.07703)
*Kanyuni Iemoto,Boris Goncharov,Gabriela Sato-Polito,Xiaoming Bi*

Main category: astro-ph.HE

TL;DR: 本文提出一种新方法，直接从半解析星族模型捕获引力波背景的完整应变概率密度，应用于SMBHB模型后独立复现了NANOGrav 15年数据的参数估计，并将PTA观测映射到"Illustris"星系并合模型上，得到$h_{\rm c}$跨度$(1$-$6)\times10^{-16}$、$N_{\rm c}$跨度$(0.3$-$7.1)\times10^{-3}$（$τ$最高8 Gyr）。


<details>
  <summary>Details</summary>
Motivation: PTA通过引力波背景观测约束超大质量双黑洞的星族性质。现有方法仅对星族合成库插值或只考虑应变谱均值，未能捕捉完整的应变概率密度，因此需要一种能直接从半解析星族模型获取完整应变概率分布的新方法。

Method: 本文提出一种新方法，直接从半解析SMBHB星族模型捕获引力波背景的完整应变概率密度，而非对库插值或仅用应变谱均值。将该方法应用于半解析SMBHB星族模型，并以源计数强度为中间计算产物，将PTA观测映射到基于"Illustris"数值流体动力学模拟的星系并合预设模型上，检验离散贡献对简并的消解。

Result: 该方法独立复现了NANOGrav合作组织用其15年数据进行的该模型参数估计，并展示了GWB中离散SMBHB贡献对星族参数空间简并的消解程度。作为原理验证，将PTA观测映射到"Illustris"模型后发现：延迟时间$τ$介于千秒差距与亚秒差距SMBHB间距之间，当$τ$最高达8 Gyr时$h_{\rm c}$跨度$(1$-$6)\times10^{-16}$，$N_{\rm c}$跨度$(0.3$-$7.1)\times10^{-3}$。

Conclusion: 作者表明该方法能够直接从半解析星族模型获得完整应变概率密度并复现NANOGrav的参数估计，证明了该方法在连接PTA观测与SMBHB星族模型上的有效性，并展示了延迟时间$τ$对特征应变及源计数的定量影响。根据摘要无法判断该方法对脉冲星或磁星观测研究的方法学可迁移性。
...
### [14] [SN 2025fhm: A central-engine powered Ic-BL supernova associated with X-ray transient EP250304a](https://arxiv.org/abs/2609.08356)
...
TL;DR: EP250304a/SN 2025fhm是一次由爱因斯坦探针发现的河外快速X射线暂现源及其关联的Ic-BL超新星。晚期光变曲线不能用放射性$^{56}$Ni衰变解释，需要中心引擎供能，最优拟合模型为自转周期$\sim$12.60 ms、磁场$\sim 3.52\times10^{15}$ G的磁星。


<details>
  <summary>Details</summary>
Motivation: 研究EFXT EP250304a及其关联的超新星SN 2025fhm的能量来源。其晚期光变曲线仅由$^{56}$Ni放射性衰变无法解释，存在显著能量盈余，因而需要判断是否存在中心引擎（如磁星）供能，以理解EFXT及其超新星的能量预算和物理起源。

Method: 摘要说明进行了X射线、光学和射电后随观测。光学观测使用了Korea Microlensing Telescope Network、Thai Robotic Telescope、Las Cumbres Observatory 1~m全球网络、Gemini南望远镜上的Gemini多目标光谱仪以及Global Supernova Network。利用一维拉格朗日辐射流体动力学程序SNEC进行网格模拟，并用磁场强度与自转周期参数化的中心引擎（快速自转高磁化中子星）模型拟合晚期热光度光变曲线及早期X射线脉冲结构。射电观测细节未说明。

Result: EP250304a的X射线光变曲线在最初$\sim$1 ks内呈现两个峰值通量相当的双宽脉冲，此特征在低光度伽马射线暴或EFXT中罕见。光学3天内的快冷却阶段可由激波茧模型拟合；超新星阶段（SN 2025fhm，3至88天）晚期光变曲线不能被$^{56}$Ni衰变单独解释，SNEC模拟显示晚期存在显著能量盈余。中心引擎模型给出最优自转周期$\sim$12.60 ms、磁场$\sim 3.52\times10^{15}$ G，能同时解释晚期热光度光变曲线与早期X射线脉冲结构。

Conclusion: 结果表明EP250304a/SN 2025fhm很可能由一个中心磁星提供能量，而非仅由放射性衰变驱动，这为EFXT及其关联超新星的能量预算和物理起源提供了新见解。根据摘要无法判断其与脉冲星、磁星单脉冲发射或FRB的直接关联。射电观测对上述结论的作用摘要未说明。
...
### [15] [Sharpening the Mass Measurement of PSR J1048+2339 via High-Cadence Photometry](https://arxiv.org/abs/2609.08456)
*Kwan-Lok Li,Ka-Yui Au,Lupin C. C. Lin,C. Y. Hui,Albert K. H. Kong,Jumpei Takata*

Main category: astro-ph.HE

TL;DR: 通过对PSR J1048+2339红背双星系统的高频次光学监测，作者利用光学掩食约束了轨道倾角并将脉冲星质量精确测定为1.79(+0.09, -0.08)太阳质量，同时估算伴星磁场为30–170 G。


<details>
  <summary>Details</summary>
Motivation: 中子星质量测量对理解其物态方程至关重要，但紧致双星中的动力学称重常受轨道倾角不确定性影响。本文旨在通过光学掩食的高频次监测改进PSR J1048+2339的质量测量精度，填补红背双星系统精确质量的空白。

Method: 利用鹿林天文台0.5米RIFT望远镜对脉冲星双星系统PSR J1048+2339进行多历元光学监测；结合掩食持续时间、文献中的档案测光数据及轨道星历表约束轨道倾角与脉冲星质量；并基于约束估算伴星磁场强度。

Result: 观测揭示频繁光学耀发与伴星引起的掩食；倾角约束为78.0±1.0度，脉冲星质量为1.79(+0.09, -0.08)太阳质量，属迄今红背系统中最精确之一；光学掩食持续时间显著短于M/GeV伽马射线测量值；估算伴星磁场强度为30–170 G。

Conclusion: 作者表明该测量是迄今红背系统中最精确的脉冲星质量之一，展示利用小口径光学望远镜改进质量测量的新方法；光学掩食短于伽马射线掩食，可能表明部分被掩食伽马射线发射源于双星内激波顶点；估算的伴星磁场与X射线轨道调制所隐含的高场强度一致。
...
### [16] [Constraints on the Low-frequency Radio Emission of the Galactic FRB Source SGR 1935+2154](https://arxiv.org/abs/2609.08668)
...
TL;DR: 在SGR 1935+2154的2020年两个活跃时段中，对$\sim$110 MHz信号作类FRB单脉冲和周期信号搜索，未发现显著类FRB脉冲，并给出爆发率上限；折叠于3.24781628 s周期时的微弱脉冲（S/N < 3.16）显著性不足以确证周期发射。


<details>
  <summary>Details</summary>
Motivation: SGR 1935+2154是已知FRB来源的银河系磁星，但对其低频射电发射了解有限。本文旨在利用LPA在$\sim$110 MHz对其FRB样单脉冲和周期信号进行搜索，填补低频端爆发率与周期辐射探测空白。

Method: 使用普希诺射电天文台大型相控阵（LPA）在$\sim$110 MHz观测；数据覆盖2020年两个活跃时段（3—5月、9—11月，每日约3.5分钟）。采用针对类FRB单脉冲和类脉冲星周期信号的新方法分析，并用蒙特卡洛模拟导出$3σ$爆发率上限，同时按已知周期折叠信号。

Result: 未发现显著类FRB脉冲。对数正态分布经蒙特卡洛得到约${10}^{1.5}~{\rm{d}}^{-1}$的$3σ$爆发率上限（$L_{ν{\rm,mean}}\sim1.3\times10^{29}$ erg s⁻¹ Hz⁻¹，$σ\sim0.85$）；幂律分布约${10}^{1.8}~{\rm{d}}^{-1}$（$β\lesssim3.0$，$L_{ν{\rm,min}}\lesssim0.7\times10^{25}$ erg s⁻¹ Hz⁻¹）。按3.24781628 s周期折叠发现微弱脉冲（S/N<3.16），不足确证；周期发射平均单色各向同性光度上限为$2.08\times10^{19}$ erg s⁻¹ Hz⁻¹。

Conclusion: 作者明确给出SGR 1935+2154的低频爆发率上限，并提示可能存在非常微弱的类脉冲星辐射。摘要未说明这些上限对FRB总体起源或磁星—FRB联系的具体物理诠释；据此无法判断其在更广层面的天体物理意义。
...
### [17] [Supermassive black hole binaries in the multi-messenger context of ground and spaceborne VLBI](https://arxiv.org/abs/2609.08704)
*B. Hudson,L. I. Gurvits,D. J. D'Orazio,C. Tiede,T. M. Eubanks,E. Mooij*

Main category: astro-ph.HE

TL;DR: 本文预测亚秒差距超大质量黑洞双星（SMBHBs）在地面和空间VLBI下的可探测性。研究发现，对流量密度受限样本，地面阵列能探测更广的参数空间（$M_\mathrm{tot}\gtrsim10^9$太阳质量可至$z=0.075$），而加入BHEX空间元素可将双星间距和位置角表征改进约$\sim4$倍。


<details>
  <summary>Details</summary>
Motivation: 引力波背景证据提示存在角间距约1–10 $μ$as的亚秒差距超大质量黑洞双星（SMBHBs）群体。本文旨在探讨下一代地面阵列和空间VLBI扩展（如BHEX）能否直接成像并表征这些SMBHBs，填补此类多信使研究中的可探测性缺口。

Method: 使用双星谱能量分布（SED）模型预测SMBHBs在VLBI下的可探测性。以Black Hole Explorer（BHEX）这一空间VLBI任务提议为案例研究，探索可探测的SMBHB参数空间，并在可见度域中识别可区分的双星特征，比较地面阵列与空间元素（含空间提供约$\sim6\,μ$as角分辨率及不受地球自转综合孔径限制的(u,v)采样）的观测能力。

Result: 对流量密度受限样本，地面阵列观测可探测更广的双星参数空间区域，$M_\mathrm{tot}\gtrsim10^9$太阳质量系统可探测至红移$z=0.075$及更远；加入BHEX等空间元素提供更精细角分辨率和不受地球自转限制的(u,v)采样，可将SMBHB间距和位置角的表征改进约$\sim4$倍。

Conclusion: 作者认为，近期未来的地面和/或空间VLBI可能实现对SMBHB的首次直接观测，并将为这类系统与脉冲星计时阵列及电磁波谱观测相结合的多信使研究作出重要贡献。该结论针对SMBHB多信使研究；摘要中提及脉冲星计时阵列作为多信使背景，未涉及磁星或FRB，因此对这些主题的可转移性根据摘要无法判断。
...
### [18] [Hierarchical Inference of the Supermassive Black Hole Binary Merger Rates from Joint Searches using Pulsar Timing Arrays](https://arxiv.org/abs/2609.09086)
*Sharon Mary Tomson,Rutger van Haasteren,Boris Goncharov*

Main category: astro-ph.HE

TL;DR: 该论文开发了一种分层贝叶斯框架，将超大质量黑洞双星并合率密度模型直接放入脉冲星计时阵列似然函数中，把单个并合信号目录建模为泊松点过程。模拟表明，单个并合信号能为随机引力波背景补充信息并收紧对并合率种群的约束。


<details>
  <summary>Details</summary>
Motivation: 脉冲星计时阵列（PTAs）通过随机引力波背景和单个SMBHB信号搜索来约束SMBHB并合率密度，但后者需用经验探测效率把源上限转为率上限。论文旨在发展一种直接推断并合率种群而非依赖探测阈值的方法。

Method: 论文开发分层贝叶斯框架，将SMBHB并合率密度模型直接置于PTA似然函数中；未知单个并合信号的目录被建模为源参数空间上的泊松点过程。描述了两种计算途径：基于固定候选源数量的目录边际似然，以及显式的跨维度采样方法（联合采样种群超参数、潜在并合信号与噪声）。先用玩具并合种群模型验证，再应用于联合预测SGWB振幅与并合目录大小的天体物理模型，并进行模拟。

Result: 模拟结果显示，相对于仅依赖随机引力波背景（SGWB），单个并合信号为SGWB补充了信息，并可以收紧对并合率种群的约束。该方法通过并合信号数量由数据推断而非靠探测阈值强加而实现。论文未在摘要中给出具体数值或显著性水平。

Conclusion: 作者的支持性结论是：该分层贝叶斯框架无需依赖经验探测效率或探测阈值，即可将SMBHB并合率密度模型直接置入PTA似然直接推断，且单个并合信号能补充SGWB信息、收紧并合率种群约束。由于摘要仅涉及引力波与超大质量黑洞双星种群，未提及磁星或FRB，无法据此判断对脉冲星、磁星或快速射电暴研究的方法可迁移性。
